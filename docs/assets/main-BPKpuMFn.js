      ...
      `,s}function hn(n){return`<span class="hlp" tabindex="0" data-tip="${String(n).replace(/"/g,"&quot;")}">?</span>`}function xT(){if(window.__helpTipInit)return;window.__helpTipInit=!0;const n=document.createElement("div");n.className="help-tip",n.style.display="none",document.body.appendChild(n);let e=null;const t=s=>{const i=s.getAttribute("data-tip");if(!i)return;clearTimeout(e),n.textContent=i,n.style.display="block";const a=s.getBoundingClientRect(),l=Math.min(260,window.innerWidth-24);n.style.width=l+"px";let c=a.left+a.width/2-l/2+window.scrollX;c=Math.max(12+window.scrollX,Math.min(c,window.scrollX+window.innerWidth-l-12)),n.style.left=c+"px";const d=n.offsetHeight;let f=a.top+window.scrollY-d-8;a.top<d+12&&(f=a.bottom+window.scrollY+8),n.style.top=f+"px"},r=()=>{e=setTimeout(()=>{n.style.display="none"},80)};document.addEventListener("mouseover",s=>{const i=s.target.closest&&s.target.closest("[data-tip]");i&&t(i)}),document.addEventListener("mouseout",s=>{s.target.closest&&s.target.closest("[data-tip]")&&r()}),document.addEventListener("focusin",s=>{const i=s.target.closest&&s.target.closest("[data-tip]");i&&t(i)}),document.addEventListener("focusout",s=>{s.target.closest&&s.target.closest("[data-tip]")&&r()}),document.addEventListener("click",s=>{const i=s.target.closest&&s.target.closest("[data-tip]");i&&(n.style.display==="block"?r():t(i))})}function RT(){if(window.__chartHoverInit)return;window.__chartHoverInit=!0;const n=e=>e.querySelectorAll("circle[data-tip]").forEach(t=>{t.setAttribute("fill","transparent"),t.removeAttribute("stroke")});document.addEventListener("mousemove",e=>{const t=e.target.closest&&e.target.closest(".ichart");if(!t)return;const r=t.querySelectorAll("circle[data-tip]");if(!r.length)return;let s=null,i=1/0;r.forEach(a=>{const l=a.getBoundingClientRect(),c=Math.abs(l.left+l.width/2-e.clientX);c<i&&(i=c,s=a)}),s&&(n(t),s.setAttribute("fill","#60a5fa"),s.setAttribute("stroke","var(--surface,#161b22)"),s.setAttribute("stroke-width","2"),s.dispatchEvent(new MouseEvent("mouseover",{bubbles:!0})))}),document.addEventListener("mouseout",e=>{const t=e.target.closest&&e.target.closest(".ichart");t&&n(t)})}const dr=n=>"£"+Math.round(n).toLocaleString();function Mp(n,e,t){return`<svg class="ichart" viewBox="0 0 ${e} ${t}" style="width:100%;height:auto;overflow:visible;display:block" role="img">${n}</svg>`}function Dp(n,{max:e,valueFmt:t=dr,tip:r,pct:s=!1}={}){const m=n.length;if(m<2)return"";const g=e??(s?100:Math.max(1,...n)),b=F=>56+F/(m-1)*590,S=F=>174-Math.max(0,Math.min(s?100:1/0,F))/g*160,I=n.map((F,U)=>`${b(U).toFixed(1)},${S(F).toFixed(1)}`).join(" "),R=`56,${174 .toFixed(1)} ${I} ${b(m-1).toFixed(1)},${174 .toFixed(1)}`,C=s?[0,50,100]:[0,g/2,g],D=[0,Math.floor((m-1)/2),m-1],B=r||((F,U)=>`Year ${U}: ${t(F)}`);return Mp(C.map(F=>`<line x1="56" y1="${S(F).toFixed(1)}" x2="646" y2="${S(F).toFixed(1)}" stroke="var(--border,#8883)" opacity=".45"/><text x="50" y="${(S(F)+3).toFixed(1)}" text-anchor="end" font-size="10" fill="var(--text-muted,#999)">${t(F)}</text>`).join("")+`<polygon points="${R}" fill="rgba(96,165,250,.13)"/><polyline points="${I}" fill="none" stroke="#60a5fa" stroke-width="2.5"/>`+n.map((F,U)=>`<circle cx="${b(U).toFixed(1)}" cy="${S(F).toFixed(1)}" r="8" fill="transparent" data-tip="${B(F,U).replace(/"/g,"&quot;")}"></circle>`).join("")+D.map(F=>`<text x="${b(F).toFixed(1)}" y="192" text-anchor="middle" font-size="10" fill="var(--text-muted,#999)">Year ${F}</text>`).join(""),660,200)}function PT(n){const l=n.p50.length;if(l<2)return"";const c=Math.max(1,...n.p90),d=I=>60+I/(l-1)*606,f=I=>222-Math.max(0,I)/c*208,m=(I,R)=>I.map((C,D)=>`${d(D).toFixed(1)},${f(C).toFixed(1)}`).concat(R.map((C,D)=>`${d(l-1-D).toFixed(1)},${f(R[l-1-D]).toFixed(1)}`)).join(" "),g=I=>I.map((R,C)=>`${d(C).toFixed(1)},${f(R).toFixed(1)}`).join(" "),b=[0,c/4,c/2,3*c/4,c],S=[0,Math.floor((l-1)/2),l-1];return Mp(b.map(I=>`<line x1="60" y1="${f(I).toFixed(1)}" x2="666" y2="${f(I).toFixed(1)}" stroke="var(--border,#8883)" opacity=".4"/><text x="54" y="${(f(I)+3).toFixed(1)}" text-anchor="end" font-size="10" fill="var(--text-muted,#999)">${dr(I)}</text>`).join("")+`<polygon points="${m(n.p90,n.p10)}" fill="rgba(96,165,250,.12)"/><polygon points="${m(n.p75,n.p25)}" fill="rgba(96,165,250,.22)"/><polyline points="${g(n.p50)}" fill="none" stroke="#60a5fa" stroke-width="2.5"/>`+n.p50.map((I,R)=>`<circle cx="${d(R).toFixed(1)}" cy="${f(I).toFixed(1)}" r="8" fill="transparent" data-tip="Year ${R}: middle ${dr(I)}; likely range ${dr(n.p10[R])} to ${dr(n.p90[R])}"></circle>`).join("")+S.map(I=>`<text x="${d(I).toFixed(1)}" y="242" text-anchor="middle" font-size="10" fill="var(--text-muted,#999)">Year ${I}</text>`).join(""),680,250)}function CT(n){if(!n||!n.funded)return"";const e=s=>(s||0).toFixed(s>=10?0:1),t=n.pctSurviveFullTerm>=80?"success":n.pctSurviveFullTerm>=50?"warning":"danger",r=n.avgHigherRateYears<1?"success":n.avgHigherRateYears<4?"warning":"danger";return`
        ...
        <div class="verdict-heroes">
          ...
          <div><span class="hero" style="font-size:30px;">${(t.coverage||0).toFixed(0)}%</span>
            <span class="hero-sub">of retirement years fully paid for ${hn("Averaged over every simulated future — a plan that funds 34 of 35 years scores 97%, so a late shortfall counts far less than an early one.")}</span></div>
        </div>
        ...
        <div class="card">
          ...
          <div class="keynums">
            <div class="keynum">
              ...
              <div class="kn-label">worst case, your money still lasts at least ${hn("In the unluckiest 1-in-10 futures, the money still lasts at least this many years.")}</div>
            </div>
            <div class="keynum">
