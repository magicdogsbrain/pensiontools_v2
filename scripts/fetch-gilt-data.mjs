...
// Equity index for the rotation trigger (drawdown vs all-time high). FRED's SP500 CSV is free,
// keyless and stable but only carries ~10 years — so the all-time high is max(fetched series,
// previous snapshot's ath): the high carries forward even if it ages out of the window.
// S&P PRICE index deliberately: the rotation evidence (research/rotation-plan-aug-2026.md) was
// measured price-vs-peak on the Shiller series; a total-return index makes new highs more often
// and would fire the trigger less.
const FRED_SP500 = 'https://fred.stlouisfed.org/graph/fredgraph.csv?id=SP500';
const YAHOO_SP500 = 'https://query1.finance.yahoo.com/v8/finance/chart/%5EGSPC?range=10y&interval=1d';

function parseFredCsv(text) {
  const rows = text.trim().split('\n').slice(1).map((l) => l.split(','));
  return rows.map(([d, v]) => ({ date: d, level: parseFloat(v) })).filter((r) => Number.isFinite(r.level));
}
function parseYahoo(text) {
  const j = JSON.parse(text); const r = j.chart && j.chart.result && j.chart.result[0];
  if (!r) throw new Error('yahoo: no result');
  const ts = r.timestamp || [], close = ((r.indicators || {}).quote || [{}])[0].close || [];
  return ts.map((t, i) => ({ date: new Date(t * 1000).toISOString().slice(0, 10), level: close[i] })).filter((x) => Number.isFinite(x.level));
}
function buildEquity(series, prevAth) {
  const last = series[series.length - 1];
  const seriesMax = series.reduce((m, r) => (r.level > m.level ? r : m), series[0]);
  const ath = Math.max(seriesMax.level, prevAth || 0);
  const athDate = ath === seriesMax.level ? seriesMax.date : 'carried forward';
  return {
    ...
    date: last.date, level: last.level, ath, athDate,
    drawdown: last.level / ath - 1
  };
}
async function refreshEquity() {
  let prevAth = 0;
  try { if (existsSync('public/data/equity.json')) prevAth = JSON.parse(readFileSync('public/data/equity.json', 'utf8')).ath || 0; } catch (e) { /* first run */ }
  let series = null;
  try { series = parseFredCsv(await fetchText(FRED_SP500)); } catch (e) { console.warn('FRED SP500 failed:', e.message); }
  if (!series || series.length < 100) {
    try { series = parseYahoo(await fetchText(YAHOO_SP500)); } catch (e) { console.warn('Yahoo ^GSPC failed:', e.message); }
  }
  if (!series || series.length < 100) { console.warn('equity index unavailable this run — keeping the previous files'); return null; }
  const eq = buildEquity(series, prevAth);
  const json = JSON.stringify(eq, null, 1);
  for (const dir of ['public/data', 'docs/data']) { mkdirSync(dir, { recursive: true }); writeFileSync(`${dir}/equity.json`, json); }
  writeFileSync('src/data/equitySnapshot.js',
    '// GENERATED by scripts/fetch-gilt-data.mjs — bundled fallback when public/data/equity.json cannot be fetched.\n'
    + '// Do not hand-edit.\nexport default ' + json + ';\n');
  console.log(`equity: ${eq.symbol} ${eq.level} (${eq.date}), ATH ${eq.ath} (${eq.athDate}), drawdown ${(eq.drawdown * 100).toFixed(1)}%`);
  return eq;
}
...
async function main() {
  ...
  await refreshEquity();   // independent of the gilt data: its own try/catch keeps failures separate
}
