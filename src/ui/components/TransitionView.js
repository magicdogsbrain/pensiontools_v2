...
export function transitionHtml(a) {
  ...
  if (stage && stage.key === 'bridge') h += '<p class="hint">You are already retired: the months until ' + esc(startLabel || 'the plan\'s year 0') + ' are the run-up, paid from your SIPP cash. The moves below are the rungs and cash to have in place before the ladder\'s first tax year — not a retirement date.</p>';
  ...
  if (ledgerCount) h += '<p class="hint" style="margin-top:8px;">What you hold comes from your <strong>My funds</strong> list (Stress tester → Settings), typed or pasted from your platform. If a line is not yours — a pasted statement can mis-read a code — remove it there and this page updates.</p>';
