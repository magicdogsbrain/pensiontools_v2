...
export function transitionHtml(a) {
  ...
  h += '<div class="alert ' + tone + '"><strong>' + prog.held + '% of the target is held</strong>' + (prog.total ? ' · ' + prog.doneCount + ' of ' + prog.total + ' moves ticked off' : ' · nothing to do') + (startLabel ? ' · plan starts ' + esc(startLabel) : '') + (seq ? ' · ' + seq.months + ' month' + (seq.months === 1 ? '' : 's') + ' to go' : '') + '.'
  ...
  if (!ledgerCount) h += '<div class="alert alert-warning">No holdings in the ledger yet — everything below reads as "to buy". Enter what you hold under <strong>My funds</strong> in the Stress tester\'s Settings (ticker or gilt code such as TR30, value, units for gilts, wrapper). <button type="button" class="risk-btn" data-on-click="openMyFunds()">Enter my holdings</button></div>';
