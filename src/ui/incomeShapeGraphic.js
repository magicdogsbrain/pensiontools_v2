...
export function incomeStaircaseSvg({ steps, ageNow, horizonAge, essentials = 0, budgetGross = 0 }, o = {}) {
  ...
  vals.forEach((v, i) => { const age = ageNow + i; const x = padL + i * colW + 1, w = Math.max(1, colW - 2); const below = essentials > 0 && v < essentials; const fill = below ? '#f97316' : (phase(age) === 'go-go' ? '#60a5fa' : phase(age) === 'go-slow' ? '#818cf8' : '#a78bfa'); s += `<rect x="${x.toFixed(1)}" y="${y(v).toFixed(1)}" width="${w.toFixed(1)}" height="${(y0 - y(v)).toFixed(1)}" fill="${fill}" opacity=".85"><title>Age ${age}: ${gbpK(v)} a year${below ? ' — below your essentials' : ''} (${phase(age)})</title></rect>`; });
  ...
  let lx = padL; for (const [c, t] of [['#60a5fa', 'go-go'], ['#818cf8', 'go-slow'], ['#a78bfa', 'no-go'], ['#f97316', 'below essentials']]) { s += `<rect x="${lx}" y="${H - 12}" width="10" height="10" fill="${c}"/><text x="${lx + 14}" y="${H - 3}" font-size="10" fill="var(--text-muted,#999)">${esc(t)}</text>`; lx += 14 + t.length * 5.6 + 18; }
